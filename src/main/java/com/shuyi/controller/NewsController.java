...
import com.shuyi.model.ResObject;
import com.shuyi.service.NewsCategoryService;
import com.shuyi.service.NewsService;
import com.shuyi.util.Constant;
import com.shuyi.util.PageUtil;
...
public class NewsController {
	...
	public String newsManage(News news,@PathVariable Integer pageCurrent,@PathVariable Integer pageSize,@PathVariable Integer pageCount, Model model,HttpSession httpSession) {
		//判断
	...
	public String newsEditGet(Model model,News news,HttpSession httpSession) {
		NewsCategory newsCategory = new NewsCategory();
		newsCategory.setStart(0);
		newsCategory.setEnd(Integer.MAX_VALUE);
		List<NewsCategory> newsCategoryList = newsCategoryService.list(newsCategory);
	...
	public ResObject<Object> newsEditState(News news) {
		...
		if(news.getCommendState()==0){
			...
			if(news.getCommendState()==1){
				news.setCommendState(2);
			}else if(news.getCommendState()==2){
				news.setCommendState(1);
			}
		}
